	...
	function writeBoard() {
        ...
        mFileAjax(sUrl, formData, function(data) {
        	if(data.resultCode == "0000") {
         		...
         		location.href = "<c:url value='/hills/board/list.do?bbsType=9'/>";
            } else {
         	   alert(data.resultMessage);
            }
        });
	...
	<div class="contents">
		...
		<form id="fileForm" action="fileUpload" method="post" enctype="multipart/form-data">
			...
			<div class="btnBox">
				...
				<a href="javascript:writeBoard()" class="orangeBtn">등록</a>
				</c:if>
				<c:if test="${actionFlag == 'update'}"> 
				<a href="javascript:writeBoard()" class="orangeBtn">수정</a>
				</c:if>
			</div>
