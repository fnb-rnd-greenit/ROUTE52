	...
	<div id="championTop">
		<div class="chTopBox">
			...
			<div class="chtopBox_con">
				...
				<ul>
					<li id="headerTab1" onclick="location.href = '<c:url value='/hills/champion/champion5.do'/>'">대회요강</li>
					<%-- <li id="headerTab2" onclick="location.href = '<c:url value='/hills/champion/trumpy5.do'/>'">시상내역</li> --%>
					<li id="headerTab2" onclick="alert('준비중입니다.');">시상내역</li>
					<c:choose>
					    <c:when test="${sessionScope.msMember.msId eq '' || sessionScope.msMember.msId eq null}">
							<li id="headerTab3" onclick="initPopup(1)">참가신청</li>
							<!-- <li id="headerTab3" onclick="alert('5/4일 부터 참가신청이 가능합니다.');">참가신청</li> -->							
					    </c:when>
					    <c:otherwise>
					...
					<%-- <li id="headerTab6" onclick="location.href = '<c:url value='/hills/champion/ladyPassport2.do'/>'">리더보드</li> --%>
					<li id="headerTab4" onclick="location.href = '<c:url value='/hills/board/list.do?bbsType=8'/>'">대회스케치</li>
					<li id="headerTab5" onclick="location.href = '<c:url value='/hills/board/list.do?bbsType=7'/>'">공지사항</li>
					<%-- <li id="headerTab8" onclick="location.href = '<c:url value='/hills/champion/kingofking.do'/>'">왕중왕전</li> --%>
					<li id="headerTab8" onclick="alert('결선 종료 후 공지예정입니다');">왕중왕전</li>
					<li id="headerTab7" onclick="location.href = '<c:url value='/hills/contents/big2final.do'/>'">BIG2대회 리뷰</li>
				</ul>
				</div>
			</div>
		</div>
	</div>
<jsp:include page="../include/popup/champion_pop5.jsp" />